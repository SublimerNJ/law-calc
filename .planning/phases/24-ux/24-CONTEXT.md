# Phase 24: 소송/법원 UX·논리 감사 - Context
...
소송/법원 카테고리 5개 계산기(lawsuit-cost, payment-order, civil-mediation, family-court, attorney-fee)에서 사용자가 정상적으로 사용할 때 논리적으로 이상하다고 느낄 수 있는 모든 포인트를 찾아 수정한다. 16개 요구사항(INPUT, RESULT, FLOW, CONSIST, EDGE)을 각 계산기에 적용.
...
- `handleNumberChange()`: attorney-fee에 숫자만 입력 헬퍼 있음 — 다른 계산기에도 적용 가능

### Established Patterns
- `useState` + `handleCalculate` 버튼 클릭 패턴 (실시간 계산 아님)
- 에러 상태: lawsuit-cost는 `error` state 사용, attorney-fee는 조용히 `return` (불일관)
...
### 현재 발견된 불일관 예시
- lawsuit-cost: 에러 메시지를 UI에 표시 (`setError`)
- attorney-fee: 에러 시 조용히 무시 (`if (!val || val <= 0) return;`)
- 이 패턴 차이가 CONSIST-03 (에러 메시지 일관성) 위반 후보
