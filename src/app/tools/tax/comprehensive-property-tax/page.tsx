  ...
  return (
    <CalculatorLayout tool={tool} category={category}>
      <div className="premium-card p-6 mb-4">
        ...
        <div className="mb-4">
          <label className="block text-sm text-gray-400 mb-2">주택 공시가격 합산 (원)</label>
          <input
            type="text"
            inputMode="numeric"
        ...
        <div className="mb-4">
          <label className="flex items-center gap-2 cursor-pointer">
            ...
            />
            <span className="text-sm text-gray-300">1세대 1주택자</span>
          </label>
        </div>
        ...
        <div className="mb-6 p-3 rounded-lg bg-[#0d1424] border border-[#1e2d4a]">
            ...
            <>
              <div className="space-y-3 mb-4">
                <div className="flex justify-between">
                  <span className="text-sm text-gray-400">공시가격 합산</span>
                  <span className="text-white">{formatNumber(result.totalAssessedValue)}원</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-gray-400">공제금액</span>
                  <span className="text-white">{formatNumber(result.deduction)}원</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-gray-400">과세표준 (공시가 x 60% - 공제)</span>
                  <span className="text-white">{formatNumber(result.taxBase)}원</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-gray-400">산출세액</span>
                  <span className="text-white">{formatNumber(result.calculatedTax)}원</span>
                </div>
                {result.seniorDeduction > 0 && (
                  <div className="flex justify-between">
                    <span className="text-sm text-gray-400">고령자공제</span>
                    <span className="text-red-400">-{formatNumber(result.seniorDeduction)}원</span>
                  </div>
                )}
                <div className="flex justify-between">
                  <span className="text-sm text-gray-400">종합부동산세</span>
                  <span className="text-white">{formatNumber(result.finalTax)}원</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-sm text-gray-400">농어촌특별세 (20%)</span>
