...
const SMALL_CLAIMS_SERVICE_ROUNDS = 10;
...
export default function SmallClaimsPage() {
  ...
  const handleCalculate = () => {
    ...
    if (!val || val <= 0) {
      setError('청구금액을 입력해 주세요.');
      return;
    }
    if (val > MAX_AMOUNT) {
      setError('소액사건은 소가 3,000만원 이하입니다. 일반 소송비용 계산기를 이용해 주세요.');
      return;
    }
    ...
    const offlineServiceFee = totalParties * SMALL_CLAIMS_SERVICE_ROUNDS * SERVICE_FEE_UNIT;
  ...
  return (
    <CalculatorLayout tool={tool} category={category}>
      <div className="premium-card p-6 mb-4">
        <h2 className="text-lg font-semibold text-slate-900 mb-4">소액사건 재판비용 계산</h2>

        <div className="mb-4">
          <label className="block text-sm text-slate-600 mb-2">청구금액 (원)</label>
          <p className="text-xs text-gray-500 mb-1">소액사건: 소가 3,000만원 이하 (소액사건심판법 제2조)</p>
          <input
            type="text"
            inputMode="numeric"
        ...
        <div className="mb-6">
          ...
          {filingMethod === 'ecourt' && (
            <p className="text-xs text-gray-500 mt-1">전자소송: 인지대 10% 할인 + 송달료 피고 수만 산정</p>
          )}
        </div>
        ...
        {error && (
          <div className="mb-4 p-3 rounded-lg bg-red-50 border border-red-200">
            ...
            {parseInt(amount.replace(/,/g, ''), 10) > MAX_AMOUNT && (
              <Link href="/tools/court/lawsuit-cost" className="text-sm text-blue-600 underline mt-1 inline-block">
                일반 소송비용 계산기로 이동
              </Link>
            )}
          </div>
        ...
        <>
          <div className="premium-card p-6 mb-4">
            <h2 className="text-lg font-semibold text-slate-900 mb-4">
              {filingMethod === 'ecourt' ? '전자소송' : '오프라인'} 소액사건 재판비용
            </h2>
              ...
              <tbody>
                ...
                <tr className="border-b border-slate-200">
                  <td className="py-3 text-sm text-slate-600">
                    ...
                    } × {SMALL_CLAIMS_SERVICE_ROUNDS}회 × {formatNumber(SERVICE_FEE_UNIT)}원)
                  </td>
                  <td className="py-3 text-right text-slate-900 font-medium">
              ...
              <tbody>
                ...
                <tr className="border-b border-slate-200/50">
                  ...
                  <td className={`py-2.5 text-right ${filingMethod === 'offline' ? 'font-semibold' : 'text-slate-600'}`}>
                    ...
                    <span className="block text-xs text-gray-400">{result.plaintiffs + result.defendants}명 × {SMALL_CLAIMS_SERVICE_ROUNDS}회</span>
                  </td>
                  <td className={`py-2.5 text-right ${filingMethod === 'ecourt' ? 'font-semibold' : 'text-slate-600'}`}>
                    {formatNumber(result.ecourtServiceFee)}원
                    <span className="block text-xs text-gray-400">피고 {result.defendants}명 × {SMALL_CLAIMS_SERVICE_ROUNDS}회</span>
                  </td>
                </tr>
                <tr>
          ...
          <div className="premium-card p-6 mb-4">
            <h2 className="text-lg font-semibold text-slate-900 mb-4">일반소송과 비교</h2>
            <div className="space-y-2">
              <div className="flex justify-between text-sm">
                <span className="text-slate-600">일반소송 송달료 (1심 15회 기준)</span>
                <span className="text-slate-900">
                  {formatNumber(
                    filingMethod === 'ecourt'
                      ? result.defendants * 15 * SERVICE_FEE_UNIT
                      : (result.plaintiffs + result.defendants) * 15 * SERVICE_FEE_UNIT
                  )}원
                </span>
              </div>
              <div className="flex justify-between text-sm">
                <span className="text-slate-600">소액사건 송달료 (5회 기준)</span>
                <span className="text-green-600 font-medium">
                  {formatNumber(filingMethod === 'ecourt' ? result.ecourtServiceFee : result.offlineServiceFee)}원
                </span>
              </div>
              <div className="flex justify-between text-sm pt-2 border-t border-slate-200">
                <span className="text-slate-900 font-semibold">송달료 절감액</span>
                <span className="text-green-600 font-semibold">
                  {formatNumber(
                    (filingMethod === 'ecourt'
                      ? result.defendants * 15 * SERVICE_FEE_UNIT - result.ecourtServiceFee
                      : (result.plaintiffs + result.defendants) * 15 * SERVICE_FEE_UNIT - result.offlineServiceFee
                    )
                  )}원
                </span>
              </div>
          ...
          <div className="premium-card p-6">
            <h2 className="text-lg font-semibold text-slate-900 mb-3">소액사건 안내</h2>
            <ul className="text-xs text-gray-500 space-y-1.5 list-disc list-inside">
              <li>소가 3,000만원 이하 민사사건에 적용 (소액사건심판법 제2조)</li>
              <li>송달료: 당사자 1인당 <strong>10회분</strong> (일반 단독사건은 110회분)</li>
              <li>1회 변론으로 심리를 종결하는 것이 원칙</li>
              <li>이행권고결정 제도로 간편하게 해결 가능</li>
              <li>변호사 없이 본인이 직접 진행 가능</li>
              <li>판결에 대해 항소 가능 (항소심은 일반소송 절차)</li>
            </ul>

            <div className="mt-4 pt-4 border-t border-slate-200">
              <p className="text-xs text-gray-500">
                인지대: 민사소송등인지법 별표 기준 (소액사건도 동일) | 송달료: 2026년 기준 1회 {formatNumber(SERVICE_FEE_UNIT)}원
              </p>
              <p className="text-xs text-gray-500 mt-1">
                소액사건 송달 횟수: 10회 (송달료규칙 별표) | 전자소송: 피고 수만 산정
              </p>
              <p className="text-xs text-gray-500 mt-1">
