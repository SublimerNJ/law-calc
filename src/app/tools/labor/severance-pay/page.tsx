...
export default function SeverancePayPage() {
  ...
  const inputClass = 'w-full bg-[#0d1424] border border-[#1e2d4a] rounded-lg px-4 py-3 text-white focus:border-[#f59e0b] focus:outline-none';
  ...
  return (
    <CalculatorLayout tool={tool} category={category}>
      <div className="premium-card p-6 mb-4">
        ...
        <div className="mb-4">
          <label className="block text-sm text-gray-400 mb-2">임금 입력 방식</label>
        ...
        {wageMode === 'monthly' ? (
          <div className="mb-6">
            <label className="block text-sm text-gray-400 mb-2">월 급여 (세전, 원)</label>
          ...
          <div className="mb-6">
            <label className="block text-sm text-gray-400 mb-2">퇴직 전 3개월 총 임금 (세전, 원)</label>
      ...
      {result !== null && (
        <>
          <div className="premium-card p-6 mb-4">
            ...
            <div className="mb-4">
              <p className="text-sm text-gray-400 mb-1">{result.eligible ? '퇴직금 (세전)' : '비례 퇴직금 (참고용, 세전)'}</p>
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div className="p-3 rounded-lg bg-[#0d1424]">
                <p className="text-xs text-gray-400 mb-1">예상 퇴직소득세</p>
              ...
              <div className="p-3 rounded-lg bg-[#0d1424]" style={{ borderLeft: `3px solid ${category.color}` }}>
                <p className="text-xs text-gray-400 mb-1">예상 실수령액</p>
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div>
                <p className="text-xs text-gray-400 mb-1">재직기간</p>
                <p className="text-sm text-white">{formatNumber(result.totalDays)}일 ({result.years}년)</p>
              </div>
              <div>
                <p className="text-xs text-gray-400 mb-1">1일 평균임금</p>
                <p className="text-sm text-white">{formatNumber(result.dailyAvgWage)}원</p>
              </div>
              <div>
                <p className="text-xs text-gray-400 mb-1">3개월 총임금</p>
                <p className="text-sm text-white">{formatNumber(result.threeMonthTotal)}원</p>
              </div>
              <div>
                <p className="text-xs text-gray-400 mb-1">3개월 역일수</p>
                <p className="text-sm text-white">{result.threeMonthDays}일</p>
              </div>
            </div>
          </div>
          ...
          <div className="premium-card p-6">
            ...
            <pre className="text-xs text-gray-300 bg-[#0d1424] p-3 rounded-lg whitespace-pre-wrap font-mono mb-4">
{`[퇴직금]
