    ...
    <CalculatorLayout tool={tool} category={category}>
      ...
      {result !== null && (
        <div className="premium-card p-6">
          ...
          <div className="mb-4">
            <p className="text-sm text-slate-600 mb-1">수당 합계</p>
          ...
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              <p className="text-sm text-slate-600 mb-1">연장근로수당</p>
              <p className="text-lg text-slate-900">{formatNumber(result.overtimePay)}원</p>
            </div>
            <div>
              <p className="text-sm text-slate-600 mb-1">야간근로수당</p>
              <p className="text-lg text-slate-900">{formatNumber(result.nightPay)}원</p>
            </div>
            <div>
              <p className="text-sm text-slate-600 mb-1">휴일근로수당</p>
              <p className="text-lg text-slate-900">{formatNumber(result.holidayPay)}원</p>
            </div>
            <div>
...
${result.overtimePay > 0 ? `\n연장근로 = ${formatNumber(result.hourlyWage)} × ${overtimeHours}h × 1.5배 = ${formatNumber(result.overtimePay)}원` : ''}${result.nightPay > 0 ? `\n야간근로 = ${formatNumber(result.hourlyWage)} × ${nightHours}h × 0.5배(가산) = ${formatNumber(result.nightPay)}원` : ''}${result.holidayPay > 0 ? `\n휴일근로 = ${formatNumber(result.hourlyWage)} × ${holidayHours}h × 1.5배${holidayOvertimeHours && parseFloat(holidayOvertimeHours) > 0 ? ` + ${formatNumber(result.hourlyWage)} × ${holidayOvertimeHours}h × 2.0배` : ''} = ${formatNumber(result.holidayPay)}원` : ''}`}
          ...
          <div className="mb-4">
            <p className="text-sm text-slate-600 mb-3">수당 배율 기준</p>
            <table className="w-full text-xs">
              <tbody>
                <tr className="border-b border-slate-200/50"><td className="py-2 text-slate-600">연장근로</td><td className="py-2 text-right text-slate-600">통상시급 × 1.5배</td></tr>
                <tr className="border-b border-slate-200/50"><td className="py-2 text-slate-600">야간근로 (22~06시)</td><td className="py-2 text-right text-slate-600">통상시급 × 0.5배 (가산)</td></tr>
                <tr className="border-b border-slate-200/50"><td className="py-2 text-slate-600">휴일근로 (8h 이내)</td><td className="py-2 text-right text-slate-600">통상시급 × 1.5배</td></tr>
                <tr><td className="py-2 text-slate-600">휴일 연장 (8h 초과)</td><td className="py-2 text-right text-slate-600">통상시급 × 2.0배</td></tr>
              </tbody>
            </table>
          </div>
      ...
      <div className="premium-card p-6">
        <h2 className="text-lg font-semibold text-slate-900 mb-4">수당 미지급 시</h2>
        <ol className="space-y-3">
          <li className="flex gap-3">
            <span className="flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-slate-900" style={{ backgroundColor: '#f59e0b' }}>1</span>
            <span className="text-sm text-slate-600">사업주에게 서면 청구</span>
          </li>
          <li className="flex gap-3">
            <span className="flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-slate-900" style={{ backgroundColor: '#f59e0b' }}>2</span>
            <span className="text-sm text-slate-600">고용노동부 신고 (1350)</span>
          </li>
          <li className="flex gap-3">
            <span className="flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-slate-900" style={{ backgroundColor: '#f59e0b' }}>3</span>
            <span className="text-sm text-slate-600">3년 이내 미지급 수당 소급 가능</span>
          </li>
          <li className="flex gap-3">
            <span className="flex-shrink-0 w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold text-slate-900" style={{ backgroundColor: '#f59e0b' }}>4</span>
            <span className="text-sm text-slate-600">근로기준법 위반 시 형사처벌 가능</span>
          </li>
        </ol>
      </div>
    </CalculatorLayout>
