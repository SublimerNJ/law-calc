  ...
  return (
    <CalculatorLayout tool={tool} category={category}>
      <div className="premium-card p-6 mb-4">
        ...
        <div className="mb-4">
          <label className="block text-sm text-gray-400 mb-2">증여재산가액 (원)</label>
          <input
            type="text"
            inputMode="numeric"
        ...
        <div className="mb-4">
          <label className="block text-sm text-gray-400 mb-2">수증자와 증여자의 관계</label>
          <select
            value={relationship}
            onChange={e => {
        ...
        {relationship === '직계존속' && (
          <div className="mb-4">
            <label className="flex items-center gap-2 cursor-pointer">
              ...
              />
              <span className="text-sm text-gray-300">수증자가 미성년자</span>
            </label>
          </div>
        )}
        ...
        <div className="premium-card p-6">
          ...
          <div className="space-y-3">
            <div className="flex justify-between">
              <span className="text-sm text-gray-400">증여재산가액</span>
              <span className="text-white">{formatNumber(result.giftAmount)}원</span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm text-gray-400">증여재산공제</span>
              <span className="text-white">-{formatNumber(result.exclusion)}원</span>
            </div>
            <div className="flex justify-between border-t border-[#1e2d4a] pt-3">
              <span className="text-sm text-gray-400">과세표준</span>
              <span className="text-white font-semibold">{formatNumber(result.taxableAmount)}원</span>
            </div>
            <div className="flex justify-between">
              <span className="text-sm text-gray-400">적용세율</span>
              <span className="text-white">{result.rate > 0 ? `${result.rate * 100}%` : '-'}</span>
            </div>
            <div className="flex justify-between border-t border-[#1e2d4a] pt-3">
              <span className="text-sm text-gray-400">납부세액</span>
